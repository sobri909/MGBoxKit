...
- (void)layoutLeftWithin:(CGFloat)limit {
  limit = self.leftWidth ? self.leftWidth : limit;

  // size and discard
  CGFloat used = [self size:self.leftItems within:limit];
  leftUsed = self.leftWidth ? self.leftWidth : used;

  // deal with item alignment
  CGFloat nudge = 0;
  switch (self.leftItemsAlignment) {
    case NSTextAlignmentCenter:
      nudge = (limit - used) / 2;
      break;
    case NSTextAlignmentRight:
  ...
  for (int i = 0; i < self.leftItems.count; i++) {
    ...
    if ([view conformsToProtocol:@protocol(MGLayoutBox)]) {
      ...
      y += box.topMargin;
      x += box.leftMargin;
      box.frame = CGRectMake(roundf(x + nudge), roundf(y), box.width, box.height);
      x += box.rightMargin;

      // better be a UIView then
    } else {
      view.frame = CGRectMake(roundf(x + nudge), roundf(y), view.width,
          view.height);
    }
    x += view.width + self.itemPadding;
  }
...
- (void)layoutRightWithin:(CGFloat)limit {
  limit = self.rightWidth ? self.rightWidth : limit;

  // size and discard
  CGFloat used = [self size:self.rightItems within:limit];
  rightUsed = self.rightWidth ? self.rightWidth : used;
...
- (void)layoutMiddleWithin:(CGFloat)limit {
  limit = self.middleWidth ? self.middleWidth : limit;

  // size and discard
  CGFloat used = [self size:self.middleItems within:limit];
  middleUsed = self.middleWidth ? self.middleWidth : used;

  NSLog(@"limit:%f used:%f fixed:%f", limit, used, middleUsed);
